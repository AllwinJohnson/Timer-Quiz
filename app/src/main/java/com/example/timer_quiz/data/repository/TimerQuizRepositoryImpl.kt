...
class FlagsChallengeRepositoryImpl(
    private val preferencesManager: PreferencesManager,
    private val gson: Gson
) : FlagsChallengeRepository {

    override suspend fun getQuestions(): Result<List<Question>> {
        return try {
            // Using the provided JSON data instead of API call
            val questionsJson = """
            {
                "questions": [
                    {
            ...
            val response = gson.fromJson(questionsJson, com.example.timer_quiz.data.remote.dto.QuestionResponseDto::class.java)
    ...
    override suspend fun getFlagUrl(countryCode: String): String {
        return "https://countryflagsapi.netlify.app/flag/${countryCode.lowercase()}.svg"
    }

    override fun getScheduledTime(): Flow<Long?> {
        return preferencesManager.getScheduledTime()
    }

    override suspend fun setScheduledTime(timestamp: Long) {
        preferencesManager.setScheduledTime(timestamp)
    }

    override suspend fun clearScheduledTime() {
        preferencesManager.clearScheduledTime()
    }
}